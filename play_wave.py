    ...
    try:
        while simulation_app.is_running():
            ...
            if robot is not None and arm_ik.initialized:
                # Use IK control (not sinusoidal test)
                USE_SIMPLE_TEST = False

                if USE_SIMPLE_TEST:
                    # Simple sinusoidal motion test
                    phase = step_count * 0.05
                    wave = math.sin(phase) * 1.0
                    actions[:, 6] = wave
                    actions[:, 18] = -wave * 0.5

                    if step_count % 100 == 1:
                        print(f"[Test] Step {step_count}: wave={wave:.3f}")
                else:
                    # Full IK control
                    arm_ik.set_target(target_pos)
                    arm_joints_abs = arm_ik.compute(robot)
                    ...
                    if step_count % 50 == 1:
                        print(f"\n[IK Debug Step {step_count}]")
                        print(f"  Target (base frame): {target_pos[0].cpu().numpy()}")

                        # Get EE position in world and base frame
                        ...
                        print(f"  Current EE (base):   {ee_pos_b.cpu().numpy()}")

                        # Position error
                        pos_error = target_pos[0] - ee_pos_b
                        print(f"  Position error:      {pos_error.cpu().numpy()}")
                        print(f"  Error magnitude:     {torch.norm(pos_error).item():.3f}m")

                        print(f"  Current arm joints:  {current_arm_joints[0].cpu().numpy()}")
                        print(f"  IK output joints:    {arm_joints_abs[0].cpu().numpy()}")

                        # Joint change
                        joint_diff = arm_joints_abs[0] - current_arm_joints[0]
                        print(f"  Joint diff:          {joint_diff.cpu().numpy()}")
